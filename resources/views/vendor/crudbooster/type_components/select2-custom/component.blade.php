					...
					@else
						@if($form['datatable_ajax'] == false)
							...
							<?php 
								...
								//additional code by Mike
								$datatable_format_tbl = explode('.',explode(',',$form['datatable_format'])[2])[0];
								//end additional code by Mike
								$select_table_pk = CRUDBooster::findPrimaryKey($select_table);
								$result = DB::table($select_table)->select($select_table.'.'.$select_table_pk,$select_title);
								if($datatable_format) {
									$result->addSelect(DB::raw("CONCAT(".$datatable_format.") as $select_title"));
									//additional code by Mike
									$result->leftjoin($datatable_format_tbl,$select_table.'.'.$datatable_format_tbl.'_id','=',$datatable_format_tbl.'.id');
									//end additional code by Mike
								}
								if($select_where) {
									$result->whereraw($select_where);
