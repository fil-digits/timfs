...
class CreateSettingsFormAccessesTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('settings_form_accesses', function (Blueprint $table) {
            $table->increments('id');
            $table->integer('cms_privileges_id', false, true)->length(10)->unsigned()->nullable();
            $table->enum('action_type', array('ADD', 'EDIT','ADD READONLY','EDIT READONLY'))->nullable();
    ...
    {
        Schema::dropIfExists('settings_form_accesses');
    }
}
